...
export default function QuotationPreview({ doc, onBack, onEdit }: Props) {
  ...
  const previewData = useMemo(() => prepareLegacyDocumentData(doc, 'quotation', global), [doc, global])
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-gray-100">
      <div className="flex items-center justify-between px-6 py-3 bg-white border-b border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-semibold text-gray-900">Preview — {previewData.fields['q-no'] || 'Quotation'}</h1>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-1.5 bg-[#1B8A7A] hover:bg-[#0F6E56] disabled:opacity-60 text-white text-sm font-semibold rounded-lg"
